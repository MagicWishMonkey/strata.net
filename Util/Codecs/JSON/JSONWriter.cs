...
namespace Strata.Util.Codecs.JSON {
    internal class JSONWriter {
        ...
        internal JSONWriter(bool UseMinimalDataSetSchema, bool UseFastGuid, bool UseExtensions, bool SerializeNulls, bool IndentOutput) {
        ...
        private void WriteObject(object obj) {
            ...
            bool append = false;
            if (useExtension) {
                WritePairFast("$type", Types.GetTypeAssemblyName(t));
                append = true;
            }

            List<Getters> g = new Types().GetGetters(t);
            foreach (var p in g) {
                if (append)
                    _output.Append(',');
        ...
        private void Indent(bool dec) {
            if (_Indent) {
                //_output.AppendLine();
                //_output.Append("\r");
                for (int i = 0; i < _current_depth; i++)
                    _output.Append("\t");
